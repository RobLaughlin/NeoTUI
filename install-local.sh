...
MASON_PKGS_DIR="$HOME/.local/share/neotui/mason/packages"
...
select_lsp_servers() {
    ...
    for server in "${lsp_order[@]}"; do
        if check_lsp_installed "$server"; then
            already_installed+=("$server")
    ...
    if [[ ${#already_installed[@]} -gt 0 ]]; then
        ...
        for server in "${already_installed[@]}"; do
            if [[ -n "$installed_list" ]]; then
                installed_list+=", "
            fi
            installed_list+="${server} (${lsp_map[$server]})"
        done
        success "Already installed: ${installed_list}"
        echo ""
    fi

    for server in "${to_prompt[@]}"; do
...
show_neotui_lf_features() {
    echo ""
    echo -e "  ${BOLD}NeoTUI lf features:${NC}"
    echo ""
    echo -e "  ${BOLD}Shell sync${NC}"
    ...
    echo -e "    ${GREEN}yy / pp${NC}     Copy / Paste file"
    echo -e "    ${GREEN}yp${NC}          Copy file path to clipboard"
    echo -e "    ${GREEN}md / mf${NC}     Create directory / file"
    echo ""
    echo -e "  ${BOLD}Opening files${NC}"
    ...
    echo -e "    ${GREEN}gh${NC}          Go to home directory"
    echo ""
}
...
configure_lf() {
    ...
    if ! grep -q "cmd sync-shell" "$lf_config" 2>/dev/null; then
        if ask_yes_no "Add sync-shell command (S)?" "Y"; then
...
LFEOF
    ...
    if ! grep -q "cmd open" "$lf_config" 2>/dev/null; then
        if ask_yes_no "Add open command (Enter/o)?" "Y"; then
            lf_additions+=$'\n'"map <enter> open"$'\n'"map o open"
        fi
    fi

    if ! grep -q "cmd open-tab" "$lf_config" 2>/dev/null; then
        if ask_yes_no "Add open-tab command (Shift+Enter/O)?" "Y"; then
            lf_additions+=$'\n'"map <s-enter> open-tab"$'\n'"map O open-tab"
        fi
    fi

    if ! grep -q "cmd toggle-preview" "$lf_config" 2>/dev/null; then
        if ask_yes_no "Add toggle-preview command (zp)?" "Y"; then
            lf_additions+=$(cat << 'LFEOF'
...
main() {
    ...
    if [[ $failures -gt 0 ]]; then
        ...
        warn "Some tools failed to install. You can re-run ./install-local.sh to retry."
    else
